...
kotlin {
    ...
    androidLibrary {
        namespace = "io.github.saifullah.nurani.ads.inmobi"
        compileSdk = 36
        minSdk = 24
    ...
    listOf(iosX64(), iosArm64(), iosSimulatorArm64()).forEach { target ->
        target.binaries.framework {
            ...
            val xcframeworkPath = projectDir.resolve("frameworks/InMobiSDK.xcframework")
            ...
            linkerOpts("-framework", "InMobiSDK", "-F$frameworkPath")
            linkerOpts("-ObjC")
        }

        target.compilations.getByName("main") {
            cinterops {
                val InMobiSDK by creating {
                    val xcframeworkPath = projectDir.resolve("frameworks/InMobiSDK.xcframework")
                    val frameworkArchDir = when (target.name) {
                        "iosArm64" -> "ios-arm64"
                        "iosX64", "iosSimulatorArm64" -> "ios-arm64_x86_64-simulator"
                    ...
                    defFile(project.file("src/nativeInterop/cinterop/InMobiSDK.def"))

                    compilerOpts("-framework", "InMobiSDK", "-F$frameworkPath")
                    compilerOpts("-I$frameworkPath/InMobiSDK.framework/Headers")
                }
            }
        }
    }
    ...
    if (isCocoapodsEnabled) {
        extensions.configure<org.jetbrains.kotlin.gradle.plugin.cocoapods.CocoapodsExtension> {
            summary = "InMobi module for MultiAds library"
...
mavenPublishing {
    ...
    pom {
        ...
        description.set("MultiAds: A professional Multiplatform Ad Management Library inmobi module.")
