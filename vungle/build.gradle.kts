...
kotlin {
    ...
    androidLibrary {
        namespace = "io.github.saifullah.nurani.ads.vungle"
        compileSdk = 36
        minSdk = 24
    ...
    listOf(iosX64(), iosArm64(), iosSimulatorArm64()).forEach { target ->
        target.binaries.framework {
            ...
            val xcframeworkPath = projectDir.resolve("frameworks/VungleAdsSDK.xcframework")
            ...
            linkerOpts("-framework", "VungleAdsSDK", "-F$frameworkPath")
            linkerOpts("-ObjC")
        }

        target.compilations.getByName("main") {
            cinterops {
                val VungleAds by creating {
                    val xcframeworkPath = projectDir.resolve("frameworks/VungleAdsSDK.xcframework")
                    val frameworkArchDir = when (target.name) {
                        "iosArm64" -> "ios-arm64"
                        "iosX64", "iosSimulatorArm64" -> "ios-arm64_x86_64-simulator"
                    ...
                    defFile(project.file("src/nativeInterop/cinterop/VungleAds.def"))

                    compilerOpts("-framework", "VungleAdsSDK", "-F$frameworkPath")
                    compilerOpts("-I$frameworkPath/VungleAdsSDK.framework/Headers")
                }
            }
        }
    }
    ...
    if (isCocoapodsEnabled) {
        extensions.configure<org.jetbrains.kotlin.gradle.plugin.cocoapods.CocoapodsExtension> {
            summary = "Vungle module for MultiAds library"
...
mavenPublishing {
    ...
    pom {
        ...
        description.set("MultiAds: A professional Multiplatform Ad Management Library vungle module.")
