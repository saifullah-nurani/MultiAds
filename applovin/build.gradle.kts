...
kotlin {
    ...
    androidLibrary {
        namespace = "io.github.saifullah.nurani.ads.applovin"
        compileSdk = 36
        minSdk = 24
    ...
    val xcf = XCFramework("applovinKit")

    listOf(iosX64(), iosArm64(), iosSimulatorArm64()).forEach { target ->
        target.binaries.framework {
            baseName = "applovinKit"
            isStatic = true
            xcf.add(this)

            val xcframeworkPath = projectDir.resolve("frameworks/AppLovinSDK.xcframework")
            ...
            linkerOpts("-framework", "AppLovinSDK", "-F$frameworkPath")
            linkerOpts("-ObjC")
        }

        target.compilations.getByName("main") {
            cinterops {
                val AppLovinSDK by creating {
                    val xcframeworkPath = projectDir.resolve("frameworks/AppLovinSDK.xcframework")
                    val frameworkArchDir = when (target.name) {
                        "iosArm64" -> "ios-arm64"
                        "iosX64", "iosSimulatorArm64" -> "ios-arm64_x86_64-simulator"
                    ...
                    defFile(project.file("src/nativeInterop/cinterop/AppLovinSDK.def"))

                    // Tell cinterop where to find the headers for this framework
                    compilerOpts("-framework", "AppLovinSDK", "-F$frameworkPath")

                    // Add specific paths so the c-interop tool knows how to resolve `#import <AppLovinSDK/AppLovinSDK.h>`
                    compilerOpts("-I$frameworkPath/AppLovinSDK.framework/Headers")
                }
            }
        }
    }
    ...
    if (isCocoapodsEnabled) {
        extensions.configure<org.jetbrains.kotlin.gradle.plugin.cocoapods.CocoapodsExtension> {
            summary = "AppLovin module for MultiAds library"
...
mavenPublishing {
    ...
    pom {
        ...
        description.set("MultiAds: A professional Multiplatform Ad Management Library applovin module.")
